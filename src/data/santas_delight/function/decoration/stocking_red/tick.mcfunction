as @e[type=item_frame,tag=santas_delight.decoration.stocking_red.create] at @s:
    ...
    summon marker ~ ~ ~ {Tags:["santas_delight.decoration.stocking_red.temp"]}
    data modify entity @n[type=marker,tag=santas_delight.decoration.stocking_red.temp] Rotation set from entity @s Rotation
    as @e[type=marker,tag=santas_delight.decoration.stocking_red.temp]:
        function santas_delight:move_to_center
        at @s positioned ^ ^-0.5 ^-0.5:
            positioned ^ ^ ^-0.39 if entity @e[type=interaction,tag=santas_delight.decoration.stocking_red.interaction,distance=..0.01] run return run function ./remove_only_self
            function ./create
        kill @s
    kill @s

as @e[type=interaction,tag=santas_delight.decoration.stocking_red.interaction] at @s:
