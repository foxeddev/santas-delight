as @e[type=item_frame,tag=santas_delight.decoration.stocking_red.create] at @s:
    ...
    as @e[type=marker,tag=santas_delight.decoration.stocking_red.temp]:
        ...
        at @s positioned ~ ~-0.5 ~:
            positioned ^ ^ ^-0.7 if entity @e[type=interaction,tag=santas_delight.decoration.stocking_red.interaction,distance=..0.01] run return run function ./remove_only_self
            function ./create
        kill @s
    kill @s
